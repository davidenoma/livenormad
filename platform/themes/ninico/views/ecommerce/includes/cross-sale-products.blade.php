...
@if($products->isNotEmpty())
    <div class="related-product-area pt-65 pb-50">
        <div class="tpsection mb-40">
            <h4 class="tpsection__title">{{ __('Buy with special price') }}</h4>
        </div>
                            ...
                            <div class="small">
                                ...
                                @if(EcommerceHelper::isCartEnabled())
                                    @if ($product->variations()->exists())
                                        <a data-id="{{ $product->slug }}" href="{{ route('public.ajax.products.shop', ['slug' => $product->slug, 'reference_product' => $parentProduct->slug]) }}" class="mt-2 btn button-quick-shop">
