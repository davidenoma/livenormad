...
app()->booted(function () {
        ...
        add_filter(SHORTCODE_REGISTER_CONTENT_IN_ADMIN, function (string $html, string $key, array $attributes) {
            ...
            $form
                ...
                ->add('alert', 'html', [
                    'html' => Blade::render(
                        sprintf(
                            '<x-core::alert title="%s"/>',
                            __('Settings in this section only works with style Grocery')
                        )
                    ),
                ])
    ...
    if (is_plugin_active('ecommerce')) {
        ...
        Shortcode::register('deal-product', __('Deal Product'), __('Deal Product'), function (ShortcodeCompiler $shortcode) {
            ...
            $flashSale = FlashSale::query()
                ->wherePublished()
                ->notExpired()
                ->when($shortcode->style !== 'list-products', function (Builder $query) {
                    $query->with('products', fn (BelongsToMany $query) => $query->inRandomOrder()->limit(1));
                })
                ->withCount('products')
            ...
            return ShortcodeForm::createFromArray($attributes)
                ...
                ->add('alert', 'html', [
                    'html' => Blade::render(
                        sprintf(
                            '<x-core::alert title="%s"/>',
                            __('Marque only works with cosmetics style.')
                        )
                    ),
                ])
    ...
    if (is_plugin_active('contact')) {
        ...
        Shortcode::setAdminConfig('contact-box', function (array $attributes) {
            $form = ShortcodeForm::createFromArray($attributes)
                ...
                ->when(is_plugin_active('contact'), function (ShortcodeForm $form) {
    ...
    Shortcode::setAdminConfig('brands', function (array $attributes) {
        ...
        return ShortcodeForm::createFromArray($attributes)
            ...
            ->add('title_color', 'customColor', [
                'label' => __('Title text color'),
            ])
            ->add('background_color', 'customColor', [
                'label' => __('Background color'),
            ])
            ->add('background_image', 'mediaImage', [
                'label' => __('Background image'),
